...
public class ValidateISBN {
	...
	private static final int SHORT_ISBN_MULTILIER = 11;
	private static final int SHORT_ISBN_LENGTH = 10;
	private static final int LONG_ISBN_LENGTH = 13;

	public boolean chekISBN(String isbn) {
	...
	private boolean isThisAValidShortISBN(String isbn) {
		...
		for(int i = 0; i < SHORT_ISBN_LENGTH; i++) {
			...
			if(!Character.isDigit(isbn.charAt(i))) {
				if (i == 9 && isbn.charAt(i) == 'X' ) {
					...
					throw new NumberFormatException("ISBN numbers can contain only numeric digits");
				}
			}
			else {
		...
		return (total % SHORT_ISBN_MULTILIER == 0);
	}
